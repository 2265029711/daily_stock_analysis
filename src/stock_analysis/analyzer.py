...
职责：
1. 封装 OpenAI 兼容 API 调用逻辑（支持 OpenAI / DeepSeek / 通义千问等）
2. 结合技术面和消息面生成分析报告
...
class OpenAIAnalyzer:
    ...
    职责：
    1. 调用 OpenAI 格式 API（OpenAI / DeepSeek / 通义千问 / Moonshot 等）进行股票分析
...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if openai_key_valid:
            ...
            logger.info("OpenAI API Key 未配置或无效，AI 分析功能不可用")
        
        if not self._openai_client:
            logger.warning("未配置有效的 OpenAI API Key，AI 分析功能将不可用")
    ...
    def analyze(
        ...
        try:
            ...
            # 获取模型名称
            model_name = getattr(self, '_current_model_name', 'unknown') or 'unknown'
            ...
            logger.info(f"[LLM调用] 开始调用 OpenAI API (temperature={generation_config['temperature']}, max_tokens={generation_config['max_output_tokens']})...")
